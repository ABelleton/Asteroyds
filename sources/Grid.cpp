...
void initGrid() {
#pragma region Creation of Walls

	grid[0][3] = new Wall(0, 3);;
	grid[15][10] = new Wall(15, 10);
	grid[3][9] = new Wall(3, 9);
	grid[12][3] = new Wall(12, 3);
	grid[4][0] = new Wall(4, 0);
	grid[5][0] = new Wall(5, 0);
	grid[10][12] = new Wall(10, 12);
	grid[11][12] = new Wall(11, 12);
#pragma endregion
...
#pragma region Creation of bases
	grid[7][5] = base(7, 5);
	grid[7][5] = base(7, 6);
	grid[7][5] = base(8, 5);
	grid[7][5] = base(8, 7);
...
#pragma region Creation of Asteroyds
	//Red asteroyds
...
#pragma region Creation of doors
...
void updateGrid() {
		...
		for (int k = 0; k < 13; k++) {
			if (grid[i][k] != nullptr) {
				if (getAstType()[0] == true) {
					Grid[i][k]->moveToken((SpaceObject::DiceValues[0] - grid[i][k]->getOrientation() - 2) % 6), grid,ships);
				}
			}
		}
	}
		...
		for (int k = 0; k < 13; k++) {
			if (grid[i][k] != nullptr) {
				if (getAstType()[1] == true) {
					Grid[i][k]->moveToken((SpaceObject::DiceValues[1] - grid[i][k]->getOrientation() - 2) % 6), grid, ships);
				}
			}
		}
	}
	...
	for (int i = 0; i < 16; i++) {
		for (int k = 0; k < 13; k++) {
			if (grid[i][k] != nullptr) {
				if (getAstType()[2] == true) {
					Grid[i][k]->moveToken((SpaceObject::DiceValues[2] - grid[i][k]->getOrientation() - 2) % 6), grid, ships);
				}
			}
		}
	}
	//Move Ship
	for (int i = 0; i < 6; i++) {
		ships[i]->moveToken((SpaceObject::DiceValues[2] - grid[i][k]->getOrientation() - 2) % 6), grid, ships)
	}
}